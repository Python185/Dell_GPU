...
def dcv_analysis():
    ...
    regression_model_method_normals = ['GPR_0','GPR_1','GPR_2','GPR_3','GPR_4','GPR_5','GPR_6','GPR_7','GPR_8','GPR_9','GPR_10','GPR_11','GPR_12']
    #regression_model_method_normals = []
    regression_model_method_GMM = ['GMR','VBGMR']
    
    number_of_test_data = 3  # 何個ずつのデータをテストに使うか
    ...
    for x_name in x_names:
        ...
        data = pd.read_csv(f'datasets/v572/{x_name}.csv', index_col=0) # データの読み込み
        data_ato = pd.read_csv('datasets/v572/x1_zr_aa.csv', index_col=0, header=0)
            ...
            for method_type in [regression_model_method_normals, regression_model_method_GMM]:
                if method_type == regression_model_method_normals:
                    ...
                    for y_num, y_name in enumerate(y_names):
                        ...
                        # DCV の fold 数を設定
                        inner_fold_number = 5
                        outer_fold_number = round(x_data.shape[0] / number_of_test_data)
                        ...
                        for regression_model_method in method_type:
                            ...
                            for train_idx, test_idx in kf.split(x_data):
                                ...
                                # 被りがないように確認
                                if x_outer.index.isin(x_inner.index).any():
