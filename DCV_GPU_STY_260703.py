            ...
            for method_type in [regression_model_method_normals, regression_model_method_GMM]:
                if method_type == regression_model_method_normals:
                    for y_num, y_name in enumerate(y_names):
                        ...
                        # DCV の fold 数を設定（外側 5-fold、行単位 KFold）
                        inner_fold_number = 5
                        ...
                        for regression_model_method in method_type:
                            ...
                            for train_idx, test_idx in kf.split(x_data):
                                x_inner = x_data.iloc[train_idx]
                                x_outer = x_data.iloc[test_idx]
                                log_y_inner = log_y.iloc[train_idx]
                                log_y_outer = log_y.iloc[test_idx]
                                ...
                                try:
                                    ...
                                    print(f'fit error in {regression_model_method}: {e}')

                                if not fit_ok:
                                    print(f'fold skipped (fit failed): {regression_model_method}')
                                    continue
                                ...
                                estimated_y_in_outer_cv.iloc[test_idx] = predicted_log_y_test.reshape([-1, 1])
                    ...
                    for _y_name in y_names:
                        _y_name_safe = sanitize_filename(_y_name)
                        predicted_y_values_df = pd.DataFrame(
                            np.empty([y_GMM.shape[0], len(method_type)]),
                            index=y_GMM.index,
                            columns=method_type,
                        )
                    ...
                    # DCV 設定（外側 5-fold、GPR 以外の通常モデルと同様に行単位 KFold）
                    ...
                    for regression_model_method in method_type:
                        ...
                        predicted_y_outer_all = np.full([y_GMM.shape[0], len(y_names)], np.nan)
                        filled_rows = np.zeros(y_GMM.shape[0], dtype=bool)

                        for train_idx, test_idx in kf_gmm.split(x_data):
                            try:
                                x_inner = x_data.iloc[train_idx]
                                y_inner = y_GMM.iloc[train_idx]
                                x_outer = x_data.iloc[test_idx]
                                ...
                                y_inner_std = y_inner.std(axis=0, ddof=1).values
                                y_inner_mean = y_inner.mean(axis=0).values
                                ...
                                if not np.all(np.isfinite(predicted_y_outer_cv)):
                                    raise ValueError('predict に非有限値が含まれます')
                                predicted_y_outer_all[test_idx] = predicted_y_outer_cv
                                filled_rows[test_idx] = True
                            except Exception as e:
                                print(f'\nGMM fold failed: {regression_model_method} ({x_name}, {tantai}) test_idx={test_idx}: {e}')

                        if not filled_rows.all():
                            missing = int((~filled_rows).sum())
                            raise ValueError(
                                f'GMM 外側 CV の予測行が不足しています: {regression_model_method} '
                                f'({x_name}, {tantai}), 未充填 {missing}/{len(filled_rows)} 行'
                            )
                    ...
                    for y_name in y_names:
                        y_name_safe = sanitize_filename(y_name)
                        pred_df = pd.concat(
                            [y_raw[y_name], predicted_y_values_dict[y_name_safe]],
                            axis=1,
                        )
                        pred_df.to_csv(
                            f'{savedir}/{y_name_safe}/predicted_y_in_DCV/{x_name}_{y_name_safe}_{tantai}_y_values_2.csv',
                            encoding='utf-8-sig',
                        )
