...
def dcv_analysis():
    ...
    for x_name in x_names:
        ...
        for tantai in tantai_name:
            ...
            if yield_predict_method == 'direct':
                ...
                drop_list= data.columns[data.columns.str.contains('選択率|収率|転化率')].to_list()
                cat_name = data['触媒ロット'].copy()
                drop_list.append('触媒ロット')                
    ...
    #                            log_y_inner = log_y[cat_name != fold_cat_outer_cv]
                        ...
                        shutil.move(f'{savedir}/{y_name}/yyplot/tempo/fig_{x_name}_{tantai}_{save_fig}.png', f'{savedir}/{y_name}/yyplot/fig_{x_name}_{tantai}_{save_fig}_gpu.png')
                        if 'GPR' not in save_fig:
                            shutil.move(f'{savedir}/{y_name}/yyplot/tempo/fig_{x_name}_{tantai}_{save_GPR_fig}.png', f'{savedir}/{y_name}/yyplot/fig_{x_name}_{tantai}_{save_GPR_fig}_gpu.png')
                        shutil.rmtree(f'{savedir}/{y_name}/yyplot/tempo/')
                        # 結果の保存
                        predicted_y_values = pd.concat([raw_y,predicted_y_values],axis=1) # Y の推定値を格納
                        predicted_y_values.to_csv(f'{savedir}/{y_name}/predicted_y_in_DCV/{x_name}_{tantai}_y_values_gup.csv', encoding= 'utf-8-sig') # y の推定値を保存
                        DCV_values.to_csv(f'{savedir}/{y_name}/prediction_accuracy/{x_name}_{tantai}_prediction_accuracy_gpu.csv') # 評価指標を保存
