...
def cv_analysis():
    ...
    y_names = ['Y(C2), %']  
    x_names = ['x1_noCe'] # x_nameはtrain_data
    test_data = ['x1_Ce'] #test_dataの説明変数はtrain_data(x_name)と同じである必要あり
    ...
    regression_model_method_normals = ['OLS']
    regression_model_method_GMM = ['GMR','VBGMR']
    use_ad = False #AD使うか否かの設定 True/False
    method_name = 'knn' #AD計算方法を、knn, ocsvm, lofから選択する
    ...
    for _y_name in y_names:
        ...
        if not os.path.exists(f'{savedir}/{_y_name}/predicted_y_in_DCV'):
            os.mkdir(f'{savedir}/{_y_name}/predicted_y_in_DCV')
        if not os.path.exists(f'{savedir}/{_y_name}/prediction_accuracy'):
        ...
        for method_type in [regression_model_method_normals, regression_model_method_GMM]:
            if method_type == regression_model_method_normals:
                for y_num, y_name in enumerate(y_names):
                    ...
                    for regression_model_method in method_type:
                        ...
                        raw_y = raw_y.iloc[:x_outer.shape[0]]  # raw_yも同じサイズに調整
            ...
            #                    return 1.0 - r2
                        ...
                        if use_ad is True:
                            ...
                            AD_df.columns = ['estimated_y_in_outer_cv' if c== 'Y(C2), %' else c for c in AD_df.columns]
                        ...
                        if use_ad is True:
                            plt.scatter(OK_df['raw_y'], OK_df['estimated_y_in_outer_cv'], c='blue', alpha=0.7, edgecolors='black', label='insideAD') # プロット
                            plt.scatter(out_df['raw_y'], out_df['estimated_y_in_outer_cv'], facecolors='none', edgecolors='red', alpha=0.7, label='outsideAD') # プロット
                        ...
                        # trainデータでの予測とMAE計算（use_ad=Trueのときのみ実行）
                        if use_ad is True:
                            predicted_log_y_train = np.ndarray.flatten(regression_model.predict(autoscaled_x_inner))
                            predicted_log_y_train = predicted_log_y_train * log_y_inner.std(ddof=1) + log_y_inner.mean()
                            
                            if log_transform:
                                predicted_y_train = expit(predicted_log_y_train)*100
                                actual_y_train = expit(log_y_inner)*100
                            else:
                                predicted_y_train = predicted_log_y_train
                                actual_y_train = log_y_inner
                            
                            # trainデータ全体のMAE計算（AD内・外を分けない）
                            train_mae_all = float(sum(abs(actual_y_train - predicted_y_train)) / len(actual_y_train))
                        else:
                            train_mae_all = np.nan
                        ...
                        if use_ad is True:
                            ...
                            # testデータのMAE計算（AD内・外別）
                            test_mae_inside = float(sum(abs(OK_df['raw_y'] - OK_df['estimated_y_in_outer_cv'])) / len(OK_df)) if len(OK_df) > 0 else np.nan
                            test_mae_outside = float(sum(abs(out_df['raw_y'] - out_df['estimated_y_in_outer_cv'])) / len(out_df)) if len(out_df) > 0 else np.nan
                        else:
                            evaluation[0,0] = float(1 - sum((raw_y - estimated_y_in_outer_cv.values.flatten()) ** 2) / sum((raw_y - raw_y.mean()) ** 2)) # R2 の計算
                            evaluation[1,0] = float(sum(abs(raw_y - estimated_y_in_outer_cv.values.flatten())) / len(raw_y)) # MAE の計算
                            evaluation[2,0] = float((sum((raw_y - estimated_y_in_outer_cv.values.flatten()) ** 2) / len(raw_y)) ** 0.5) # RMSE の計算         
                            ...
                            # testデータのMAE計算（AD使わない場合）
                            test_mae_inside = evaluation[1,0]
                            test_mae_outside = np.nan
                        
                        # testデータ全体のMAE計算（AD内・外を区別しない）
                        if use_ad is True:
                            # AD判定後のデータを使用
                            test_mae_all = float(sum(abs(AD_df['raw_y'] - AD_df['estimated_y_in_outer_cv'])) / len(AD_df))
                        else:
                            # AD判定なしの場合
                            test_mae_all = float(sum(abs(raw_y - estimated_y_in_outer_cv.values.flatten())) / len(raw_y))
                        
                        # MAE結果をDataFrameに保存
                        mae_results = pd.DataFrame({
                            'Dataset': ['Train_All', 'Test_Inside_AD', 'Test_Outside_AD', 'Test_All'],
                            'MAE': [train_mae_all, test_mae_inside, test_mae_outside, test_mae_all],
                            'Model': [regression_model_method] * 4,
                            'Target': [y_name] * 4,
                            'Test_Data': [test_data[0]] * 4
                        })
                        
                        # MAE結果をCSVファイルに保存
                        #mae_results.to_csv(f'{savedir}/{y_name}/prediction_accuracy/{test_data[0]}_{regression_model_method}_mae_results.csv', index=False)
