...
def cv_analysis():
    ...
    y_names = ['Y(C2), %']  
    x_names = ['x1_int_train_s'] # x_nameはtrain_data
    test_data = ['x1_int_test_s'] #test_dataの説明変数はtrain_data(x_name)と同じである必要あり
    ...
    for x_name in x_names:
        ...
        try:
            data = pd.read_csv(f'MyWork/datasets/larger_data/data_for_article/{x_name}.csv', encoding= 'utf-8-sig', index_col=0) # データの読み込み
        except UnicodeDecodeError:
            data = pd.read_csv(f'MyWork/datasets/larger_data/data_for_article/{x_name}.csv', encoding= 'cp932', index_col=0) # データの読み込み
        ...
        test_raw_data = pd.read_csv(f'MyWork/datasets/larger_data/data_for_article/{test_data[0]}.csv', encoding= 'utf-8-sig', index_col=0)
        ...
        for method_type in [regression_model_method_normals, regression_model_method_GMM]:
            if method_type == regression_model_method_normals:
                for y_num, y_name in enumerate(y_names):
                    ...
                    for regression_model_method in method_type:
                        ...
                        # 元のnumpy配列を保存（ARDカーネルモデルでtensor変換後も通常のモデルで使用するため）
                        autoscaled_x_inner_original = autoscaled_x_inner.copy()
                        autoscaled_log_y_inner_original = autoscaled_log_y_inner.copy()
                        ...
                        # GPR_3
                            ...
                            kernel.base_kernel.lengthscale = torch.ones(x_inner.shape[1])
                        # GPR_4
                        ...
                        # GPR_12
                            ...
                            kernel.outputscale = 1.0
                            kernel.base_kernel.kernels[0].lengthscale = torch.ones(x_inner.shape[1])                          
            ...
            #                    return 1.0 - r2
                        ...
                        if regression_model_method in ard_kernel_models:
                            # モデル構築
                            try:
                                # tensor変換（元のnumpy配列はautoscaled_x_inner_originalに保存済み）
                                ...
                                likelihood.noise = 0.01  # WhiteKernelのデフォルトに対応
                                ...
                                likelihood.train()
                                # 最適化の初期値設定
                                initial_lr = 0.05  # GPyTorchで一般的な値に調整
                                min_lr = 0.0001  # より低い値まで探索可能に
                                lr = initial_lr
                                ...
                                patience = 60  # training_iterationsの20%程度が一般的な目安です
                                min_delta = 1e-4 #改善とみなされる最小の変化
                                best_loss = np.inf
                                epochs_no_improve = 0
                            ...
                            # モデル構築
                            # autoscaled_x_innerがtensorの場合は元のnumpy配列を使用
                            if isinstance(autoscaled_x_inner, torch.Tensor):
                                x_for_fit = autoscaled_x_inner_original
                                y_for_fit = autoscaled_log_y_inner_original
                            else:
                                x_for_fit = autoscaled_x_inner
                                y_for_fit = autoscaled_log_y_inner
                            regression_model.fit(x_for_fit, y_for_fit)

                            #　外側のデータの予測
                            # autoscaled_x_outerがtensorの場合はnumpy配列に変換
                            if isinstance(autoscaled_x_outer, torch.Tensor):
                                x_for_predict = autoscaled_x_outer.numpy()
                            else:
                                x_for_predict = autoscaled_x_outer
                            predicted_log_y_test = np.ndarray.flatten(regression_model.predict(x_for_predict)) #　予測
                            predicted_log_y_test = predicted_log_y_test * log_y_inner.std(ddof=1) + log_y_inner.mean() #スケールを戻す
                        ...
                        plt.savefig(f'{savedir}/{y_name}/yyplot/tempo/fig_{test_data[0]}_{regression_model_method}_test_GPU.png',bbox_inches = 'tight') # 図の保存
                        #plt.show() # 図の描画
                    ...
                    if use_ad is True:
                        ...
                        try:
                            if regression_model_method in ard_kernel_models:
                                # ARDカーネルモデルの場合（tensorを使用）
                                regression_model.eval()
                                likelihood.eval()
                                with torch.no_grad(), gpytorch.settings.fast_pred_var(),gpytorch.settings.cholesky_jitter(1e-6):
                                    train_pred = likelihood(regression_model(autoscaled_x_inner))
                                    predicted_train_y = train_pred.mean.numpy()
                            else:
                                # 通常のモデルの場合（numpy配列を使用）
                                # autoscaled_x_innerがtensorの場合は元のnumpy配列を使用
                                if isinstance(autoscaled_x_inner, torch.Tensor):
                                    x_for_predict = autoscaled_x_inner_original
                                else:
                                    x_for_predict = autoscaled_x_inner
                                predicted_train_y = np.ndarray.flatten(regression_model.predict(x_for_predict))
                            
                            # スケールを戻す
                            predicted_train_y = predicted_train_y * log_y_inner.std(ddof=1) + log_y_inner.mean()
                            train_mae = float(sum(abs(log_y_inner.values - predicted_train_y)) / len(log_y_inner))
                            mae_results['train_MAE'] = train_mae
                        except Exception as e:
                            print(f"Error in train MAE calculation for {regression_model_method}: {e}")
                            mae_results['train_MAE'] = np.nan
                        
                        # testデータ全体のMAE
                        test_mae = float(sum(abs(raw_y.values - estimated_y_in_outer_cv.values.flatten())) / len(raw_y))
                        mae_results['test_all_MAE'] = test_mae
                        
                        # AD内外のMAE
                        if 'AD_df' in locals():
                            if OK_df.shape[0] >= 1:
                                test_ad_in_mae = float(sum(abs(OK_df['raw_y'] - OK_df['estimated_y_in_outer_cv'])) / len(OK_df))
                                mae_results['test_AD_in_MAE'] = test_ad_in_mae
                            else:
                                mae_results['test_AD_in_MAE'] = np.nan
                                
                            if out_df.shape[0] >= 1:
                                test_ad_out_mae = float(sum(abs(out_df['raw_y'] - out_df['estimated_y_in_outer_cv'])) / len(out_df))
                                mae_results['test_AD_out_MAE'] = test_ad_out_mae
                            else:
                                mae_results['test_AD_out_MAE'] = np.nan
                        else:
                            mae_results['test_AD_in_MAE'] = np.nan
                            mae_results['test_AD_out_MAE'] = np.nan
                        
                        # MAE結果をDataFrameとして保存
                        mae_df = pd.DataFrame([mae_results], index=[regression_model_method])
                        if 'all_mae_results' not in locals():
                            all_mae_results = mae_df
                        else:
                            all_mae_results = pd.concat([all_mae_results, mae_df])
                            
                print()
                #図の出力
                if use_ad is True and 'OK_df' in locals():
                    if OK_df.shape[0] <=1:
                    ...
                    save_fig = DCV_values.loc[:, 'r2_DCV'].idxmax()
                shutil.move(f'{savedir}/{y_name}/yyplot/tempo/fig_{test_data[0]}_{save_fig}_test_GPU.png', f'{savedir}/{y_name}/yyplot/fig_{test_data[0]}_{save_fig}_test_GPU.png')
                ...
                predicted_y_values.to_csv(f'{savedir}/{y_name}/predicted_y_in_DCV/{test_data[0]}_y_values_GPU.csv') # y の推定値を保存
                DCV_values.to_csv(f'{savedir}/{y_name}/prediction_accuracy/{test_data[0]}_prediction_accuracy_test_GPU.csv') # 評価指標を保存
                
                # MAE結果を保存
                #all_mae_results.to_csv(f'{savedir}/{y_name}/prediction_accuracy/{test_data[0]}_MAE_results.csv') # MAE結果を保存
