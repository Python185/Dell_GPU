...
#xo1 = pd.read_csv('MyWork/datasets/xo1.csv', encoding= 'cp932', index_col= 0, header= 0)
#x1_tr, x1_te = train_test_split(xo1, test_size= 0.25, shuffle= True, random_state= 42)
#x1_tr.to_csv('MyWork/datasets/x1_tr.csv', encoding= 'cp932')
#x1_te.to_csv('MyWork/datasets/x1_te.csv', encoding= 'cp932')
#x2_15 = pd.read_csv('MyWork/datasets/x2_15.csv', index_col= 0, header= 0)
#x2_15tr, x2_15te = train_test_split(x2_15, test_size= 0.25, shuffle= True, random_state= 42)
#x2_15tr.to_csv('MyWork/datasets/x2_15tr.csv', encoding= 'cp932')
#x2_19 = pd.read_csv('MyWork/datasets/x2_19.csv', index_col= 0, header= 0)
#x2_19tr, x2_19te = train_test_split(x2_19, test_size= 0.25, shuffle= True, random_state= 42)
#x2_19tr.to_csv('MyWork/datasets/x2_19tr.csv', encoding= 'cp932')

def cv_analysis():
    
    #ロジット変換、回帰条件の設定True
    log_transform = False    # False:対数変換なし　True:対数変換あり
    y_names = ['Y(C2), %']   #収率以外とするときは、98-99行の設定を変える必要あり
    #x_names = ['x26_ETA3','x2_17','x2_18'] # x の設定
    x_names = ['x4_rf1'] # x の設定
    ...
    regression_model_method_normals = ['OLS','PLS','RR','LASSO','NLSVR','RF','GPR_0','GPR_1','GPR_2','GPR_3','GPR_4',
                                        'GPR_5','GPR_6','GPR_7','GPR_8','GPR_9','GPR_10','GPR_11','GPR_12','GBDT','XGB','LGB']
    #regression_model_method_normals = ['RF','GPR_1']
    regression_model_method_GMM = ['GMR','VBGMR']
    
    #outer_fold_number = 10  # 外側の分割数。触媒が同じで評価方法が異なるサンプルをひとまとまりとして分割
    random_state = 43  # 分割する際の乱数のシード。固定すれば再現性あり
    ...
    for x_name in x_names:
        ...
        data =  pd.read_csv(f'MyWork/datasets/larger_data/{x_name}.csv', index_col=0) # データの読み込み
        drop_list= [s for s in data.columns.to_list() if _y_name in s]               
        #drop_list= data.columns[data.columns.str.contains(_y_name)].to_list()
        ...
        for method_type in [regression_model_method_normals, regression_model_method_GMM]:
            if method_type == regression_model_method_normals:
                for y_num, y_name in enumerate(y_names):
                    ...
                    DCV_values = pd.DataFrame(DCV_values,index=regression_model_method_normals,columns=['r2', 'MAE', 'RMSE','WRMSE'])
                    ...
                    for regression_model_method in method_type:
                        ...
                        x_inner, x_outer = train_test_split(x_data, test_size= 0.25, shuffle= True, random_state= random_state)
                        log_y_inner, log_y_outer = train_test_split(log_y, test_size= 0.25, shuffle= True, random_state= random_state)
                        ...
                        estimated_y_in_outer_cv = estimated_y_in_outer_cv.reindex(x_outer.index)
            ...
            #                    return 1.0 - r2
                        ...
                        estimated_y_in_outer_cv.loc[:, _y_name] = predicted_log_y_test.reshape([-1, 1]) #　データを格納

                        #　予測値の格納
                        if log_transform:
                            estimated_y_in_outer_cv = expit(estimated_y_in_outer_cv)*100
                        #predicted_y_values = predicted_y_values.iloc[: x_outer.shape[0], :]
                        predicted_y_values = predicted_y_values.reindex(x_outer.index)                        
                        predicted_y_values[regression_model_method] = estimated_y_in_outer_cv.values

                        #　yyplot
                        plt.rcParams['font.size'] = 22
                        plt.figure(figsize=figure.figaspect(1)) # 正方形
                        #plt.title(f'{x_name}') # タイトル
                        plt.scatter(raw_y, estimated_y_in_outer_cv, c='blue', alpha=0.7, edgecolors='black', s= 60) # プロット
                        ...
                        #plt.plot([-0.2, 20], [-0.2, 20], 'k-') # 対角線の描画
                        #plt.ylim(-0.2, 20) # Y のサイズ
                        #plt.xlim(-0.2, 20) # X のサイズ
                        #plt.xticks([0, 5, 10, 15, 20])
                        
                        plt.plot([-0.2, 40], [-0.2, 40], 'k-') # 対角線の描画
                        plt.ylim(-0.2, 40) # Y のサイズ
                        plt.xlim(-0.2, 40) # X のサイズ
                    ...
                    print()
                    #図の出力
                    save_fig = DCV_values.loc[:, 'r2'].idxmax()
