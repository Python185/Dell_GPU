...
def dcv_analysis():
    ...
    #regression_model_method_normals = ['OLS','PLS']
    ...
    for x_name in x_names:
        ...
        x_raw = data.drop('Result', axis= 1).copy()
        y_all = data[y_names].copy() # yの選択

        # 同じ値を多く持つ候補を削除
        threshold_of_rate_of_same_value = 0.97
            ...
            if method_type == regression_model_method_normals:
                for y_num, y_name in enumerate(y_names):
                    ...
                    for regression_model_method in method_type:
                        ...
                        min_number = math.floor(x_data.shape[0] / outer_fold_number)
                        mod_number = x_data.shape[0] - min_number * outer_fold_number
                        ...
                        for fold_number_in_outer_cv in np.arange(1, outer_fold_number + 1, 1):
                            ...
                            log_y_inner = log_y.loc[fold_index_in_outer_cv != fold_number_in_outer_cv].copy()
                ...
                #                    return 1.0 - r2
                            ...
                            estimated_y_in_outer_cv[fold_index_in_outer_cv == fold_number_in_outer_cv] = predicted_log_y_test.reshape([-1, 1])  # 格納
                        ...
                        plt.savefig(f'{savedir}/{y_name}/yyplot/fig_{x_name}{regression_model_method}.png',bbox_inches = 'tight') # 図の保存
                        plt.show() # 図の描画
...
#                        plt.plot([-0.2, 3.2], [-0.2, 3.2], 'k-')
#                        plt.ylim(-0.2, 3.2) # 縦軸の範囲
#                        plt.xlim(-0.2, 3.2)                        
