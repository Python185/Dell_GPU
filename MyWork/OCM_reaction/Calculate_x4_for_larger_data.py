...
def Boruta_Apply(target, df, perc):
    ...
    # 相関係数の大きい特徴量を削除
    x = delete_high_corr(x, threshold=0.95)
...
def Calc_x3(target, x2, metal_x, element_data, oxide_data, selector, cross_term, compo, perc):
    #metal_xの列名の変更
    ...
    ratio_columns = [col for col in metal_x.columns if 'ratio' in col]
    ...
    xenonpy_base = element_data.loc[element_data.index.isin(element_list), :]
    oxide_base = oxide_data.loc[oxide_data.index.isin(element_list), :]
    if xenonpy_base.isna().any().any() or oxide_base.isna().any().any():
    ...
    if cross_term:
        x3_oxidecross = pd.DataFrame(index= metal_x.index)
    ...
    inf_check = desc_for_boruta.isin([np.inf, -np.inf]).sum()
    if inf_check.any():
        desc_for_boruta = desc_for_boruta.replace([np.inf, -np.inf], np.nan)
        desc_for_boruta = desc_for_boruta.dropna(axis= 1)
        #print('Dropped columns containing inf:' + str(inf_check.sum(axis=0)))
...
# 北大の大きい方のデータを用いて、Cation系のみに限定して解析してみる
# OCM反応で金属は全て酸化されているという話もあるため、酸化物、金属で分離せず全ての特徴量を計算する
# データの読込み
...
data = pd.read_excel('MyWork/datasets/original_data/OCM.xlsx', index_col=0, header=0)    #C2 yield=0を削除
...
# Anion, Promotor, Supportありのデータを削除
data = data[pd.isna(data['Anion 1'])]
data = data[pd.isna(data['Promotor'])]
data = data[pd.isna(data['Support 1'])]

# 不要な列を削除
drop_col = [c for c in data.columns if 'Anion' in c or 'Promotor' in c or 'Support' in c]
data = data.drop(drop_col, axis= 1)
drop_col_2 = [c for c in data.columns if 'S(' in c or 'X' in c]
drop_col_2 = drop_col_2 + ['Preparation']
data = data.drop(drop_col_2, axis= 1)
...
# x2作成のためのmetal_x, metal_list作成
metal_ratio_col = [c for c in data.columns if 'Cation' in c]
metal_x = data[metal_ratio_col]
metal_col = [s for s in metal_x.columns if 'mol%' not in s]
metal_list = metal_x[metal_col].values.flatten()
metal_list = pd.Series(metal_list).dropna().to_list()
metal_list = list(set(metal_list))
metal_list.sort()

# metal_listが63元素と多いため、住友化学使用元素との差を確認
scc_metals = pd.read_csv('results/20230316_使用元素リスト.csv', encoding= 'utf-8-sig')
scc_metals = scc_metals.columns.tolist()
scc_metals = [s for s in scc_metals if s != 'Te']
additional_metals = [s for s in metal_list if s not in scc_metals]
additional_metals = additional_metals

# scc_metalsに含まれる元素のみに限定するとどの位減るか？
scc_metal_x = metal_x[[c for c in metal_x.columns if 'mol%' not in c]].copy()
scc_metal_x['scc'] = scc_metal_x.isin(additional_metals).any(axis=1)

# 1989→1841にしか減らなかったのでscc_metals元素のみに更新する
metal_list = scc_metals
metal_x = metal_x[metal_x.index.isin(scc_metal_x[scc_metal_x['scc'] == False].index)]
data = data[data.index.isin(scc_metal_x[scc_metal_x['scc'] == False].index)]

# 酸化物データのないRe, Pt, Au, Pdを含むデータも削除する 1841→1832となる
drop_list = ['Re', 'Pt', 'Au', 'Pd']
cation_col = [c for c in metal_x.columns if 'Cation' in c and 'mol%' not in c]
metal_x = metal_x[~metal_x[cation_col].apply(lambda row: any(el in row.values for el in drop_list), axis=1)]
data = data[data.index.isin(metal_x.index)]

# x2の作成
metal_df = pd.DataFrame(0, index=metal_x.index, columns=metal_list).astype(float)
for index, row in metal_x.iterrows():
    for col in metal_x.columns:
        if pd.notna(row[col]) and 'mol%' not in col:
            metal_df.at[index, row[col]] = row[col + ' mol%']

# metal_df内容確認
metal_df['sum'] = metal_df.sum(axis=1)
metal_df = metal_df.drop('sum', axis=1)

x2 = pd.concat([data[target], data.loc[:, 'Temperature, K': ], metal_df], axis=1)
nan_checker = x2.isna().sum()

# x2のnanを含む行を削除した結果、N=1646→Au,Re,Pt,Pd含むデータ除いてN=1637
x2 = x2.dropna(how='any', axis=0)
metal_x = metal_x.loc[metal_x.index.isin(x2.index)]
#x2.to_csv('MyWork/datasets/larger_data/x2.csv', encoding= 'utf-8-sig')
#metal_x.to_csv('MyWork/datasets/larger_data/metal_x.csv', encoding= 'utf-8-sig')

# 元素群縛り
#used_elements = ['Li','Na','K','Rb','Cs','Mg','Ca','Sr','Ba','Sc','Y'] + lanthanoid

# used_elementsの元素に限定
#elements_total = x2.loc[:, 'Li':].columns.tolist()
#unused_elements = [el for el in elements_total if el not in used_elements]
#x2 = x2[x2[unused_elements].sum(axis=1) == 0]
...
# debugのためx2, metal_xの短縮
#x2 = x2.iloc[:300, :]
#metal_x = metal_x.iloc[:300, :]
...
drop_data_list = ['space group', 'structure','oxide','hhi_p','hhi_r','CAS']
drop_data_list = [s for s in drop_data_list if s in element_data.columns]
element_data = element_data.drop(drop_data_list, axis= 1)
...
x4.to_csv('MyWork/datasets/larger_data/x4_b.csv', encoding= 'utf-8-sig')
importance_df.to_csv('MyWork/datasets/larger_data/importance_x4_b.csv', encoding= 'utf-8-sig')
