...
# CSVデータのパスとラベル列名を指定
data = pd.read_csv('datasets/v572/x1_ce_m.csv', encoding="utf-8-sig", index_col=0, header=0)
label_column = data.drop(['選択率NPA','収率NPA','触媒ロット'], axis=1).columns.tolist()
#label_column = ['選択率NPA','収率NPA','触媒ロット']

# マスク率 0.75 で CsvDataSvm をインスタンス化
csv_svm = CsvDataSvm(data, label_column=label_column, mask_ratio=0.75)

accuracy_score = csv_svm.train_eval(np.array([False] * csv_svm._num_features))
...
# ブラックボックス関数を作成
blackbox_func = make_blackbox_func(mnist_svm=csv_svm)
...
# 初期教師データを生成
n0 = 10
x, y = init_training_data(n0, csv_svm, blackbox_func)

# FMQAサイクルを実行
n = 10
for i in range(n):
    print(f"FMQA cycle {i}")
    model = TorchFM(csv_svm._num_features, k=20)
    corrcoef = train(x, y, model)
    x_hat = anneal(model, get_constraint)
    y_hat = blackbox_func(x_hat)
    x = np.vstack((x, x_hat))
