...
# virtual_dataの読込み
#metal_data = pd.read_csv('result/virtual_data/x1_av_True_metal_x_data_for_inverse_analysis.csv', encoding='utf-8-sig', index_col=0, header=0)
#xenonpy_data = pd.read_csv('result/virtual_data/x1_av_True_x_data_for_inverse_analysis_xenon_vals.csv', encoding='utf-8-sig', index_col=0, header=0)
#process_data = pd.read_csv('result/virtual_data/x1_av_True_x_data_for_inverse_analysis_exp_vals.csv', encoding='utf-8-sig', index_col=0, header=0)

# metal_xをx2様式に変換
def transform_x2(metal_x):
    def row_to_dict(row):
        d = {}
        # metal1～metal5, ratio1～ratio5 を対応付ける
        for i in range(1, 6):
            metal = row[f"metal{i}"]
            ratio = row[f"ratio{i}"]
            d[metal] = ratio
    ...
    # 各行ごとに辞書を作成し、それをリスト化
    list_of_dicts = metal_x.apply(row_to_dict, axis=1).tolist()
    # 辞書リストから DataFrame を作成（行ごとに異なる元素がキーとなる）
    x2 = pd.DataFrame(list_of_dicts)
    # 存在しない元素は NaN となるので 0 に置換
    x2 = x2.fillna(0)
    # 列名（元素記号）をアルファベット順に並べ替え
...
#elements = sorted(['Ag', 'Au', 'Cu', 'Fe', 'Zn', 'Pb', 'Ni', 'Co', 'Sn', 'Al'])
#n_elements = len(elements)
"""
def sample_composition():
    # Dirichlet分布でサンプルし、合計が1になるように正規化
    comp = np.random.dirichlet(np.ones(n_elements), 1)[0]
    # 0.01刻みに丸める
    comp_rounded = np.round(comp, 2)
    diff = 1 - comp_rounded.sum()
    # 丸め誤差がある場合、最も大きな値を調整
    if diff != 0:
        idx = np.argmax(comp_rounded)
        comp_rounded[idx] += diff
    return comp_rounded
"""
"""
# N個のサンプルを一度に生成させる
...
samples = []
while len(samples) < N:
    sample = generate_sample(metal_list)
    if sample is not None:
        samples.append(sample)

# DataFrame化
x2 = pd.DataFrame(samples)
"""
...
continuous_scaled = continuous_data.apply(zscore)
...
composition_scaled = composition_data.apply(zscore)
...
optimal_continuous_original = optimal_continuous_scaled * cont_stds + cont_means
...
optimal_composition_original = optimal_composition_scaled * composition_stds + composition_means
...
# その他微調整等
optimal_design[continuous_cols] = optimal_design[continuous_cols].round(0)
...
optimal_design_ce.to_csv('result/virtual_data/candidates_ce.csv', encoding='utf-8-sig')
optimal_design_ti.to_csv('result/virtual_data/candidates_ti.csv', encoding='utf-8-sig')
optimal_design_zr.to_csv('result/virtual_data/candidates_zr.csv', encoding='utf-8-sig')
