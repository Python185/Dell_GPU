...
fold_number = 5
save_dir = 'SEM_data/20240709/autoencoder_data/'
...
# data入力
ftir_all = pd.read_csv('SEM_data/20240709/datasets/ftir_all.csv', encoding='utf-8-sig', index_col=0, header=0)
ftir_all4 = pd.read_csv('SEM_data/20240709/datasets/ftir_all4.csv', encoding='utf-8-sig', index_col=0, header=0)
sel_yid = pd.read_csv('SEM_data/20240709/datasets/sel_yid.csv', encoding='utf-8-sig', index_col=0, header=0)
sel_yid = sel_yid.rename(columns=lambda x: x.rstrip('※※'))
sel_yid['収率NPA'] = sel_yid['選択率NPA'] * sel_yid['転化率'] / 100
sel_yid = sel_yid.drop('転化率', axis=1)

ftir_range_s = '1700.94'
ftir_range_e = '2300.7'
data = ftir_all
ftir_data = data.loc[:, ftir_range_s: ftir_range_e]
xrf = ftir_all4.loc[:, 'Si':]
data = pd.concat([sel_yid, ftir_data, xrf], axis=1)

# 収率の目標は選択率の1/10とする
#data = data.drop('収率NPA', axis=1)
#data = data.drop('選択率NPA', axis=1)
# 下2行はGMR, VBGMRの回帰するとき以外はコメントアウトで可
#data = pd.concat([data, ftir_all4['触媒ロット']], axis=1)
#data.to_csv('SEM_data/20240709/datasets/ftir_sy3.csv', encoding='utf-8-sig')
...
"""
best_dae_structures = [128, 64, 32, 16, 32, 64, 128]
...
# Autoencoder結果の読取り
#encoded_ftir_train = pd.read_csv(save_dir+'encoded_data_train.csv', encoding='utf-8-sig', index_col=0, header=0)
#encoded_ftir_test = pd.read_csv(save_dir+'encoded_data_test.csv', encoding='utf-8-sig', index_col=0, header=0)
encoded_ftir_train = encoded_data_train_df.copy()
encoded_ftir_test = encoded_data_test_df.copy()
encoded_ftir = pd.concat([encoded_ftir_train, encoded_ftir_test], axis=0)
encoded_ftir.columns = encoded_ftir.columns.astype(str)

# 結果を新しいdataとする
encoded_data = pd.concat([sel_yid, encoded_ftir, xrf, ftir_all4['触媒ロット']], axis=1)
...
irxrf_col = [c for c in encoded_data.columns if 'NPA' not in c]
...
estimated_x_test = pd.DataFrame(columns=irxrf_col, index=target_df.index)
target_dfs = {}
...
for i in range(3):
    ...
    input_index = target_dfs[f'target_df_{i}'].index
    estimated_x_test.loc[input_index, irxrf_col] = estimated_X_test
...
ftir_train = variables_train_df.loc[:, ir_col]
ftir_test = estimated_x_test.loc[:, ir_col]
autoscaled_ftir_test = (ftir_test - ftir_train.mean(axis = 0)) / ftir_train.std(axis = 0, ddof = 1)
array_data = autoscaled_ftir_test.values.astype(np.float32)
ftir_test_tensor = torch.tensor(array_data, dtype=torch.float32).to(device)
model.eval()
with torch.no_grad():
    decoded_data_test = model.decoder(ftir_test_tensor)
