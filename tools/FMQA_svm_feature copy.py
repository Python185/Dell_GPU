...
class TabularSvm:
    def __init__(
        self, mask_ratio: float, train_size=0.7, test_size=0.3, seed=0
    ) -> None:
        """与えられたマスク率に応じてクラスを初期化
        Args:
            mask_ratio (float): マスクする特徴量の割合
            train_size (float): 学習データの割合（0.0-1.0）
            test_size (float): テストデータの割合（0.0-1.0）
            seed (int): 乱数シード
        """
        # 表形式データの読み込み（例：irisデータセット）
        # CSVファイルからデータを読み込む
        data = pd.read_csv("datasets/v582/x1_ce_m.csv", encoding="utf-8-sig", header=0, index_col=0)
        
        # 特徴量とラベルを分割
        self._data = data.iloc[:, :-1]  # 最後の列以外を特徴量として使用
        self._label = data.iloc[:, -1]  # 最後の列をラベルとして使用
        self._data = (self._data - self._data.mean()) / self._data.std()  # 標準化
        self._model = svm.SVC()
        self._num_features = len(self._data.columns)
        self._num_masked_features = max(1, int(self._num_features * mask_ratio))  # 最低1つの特徴量はマスク
        ...
        np.random.seed(seed)
        self._mask = np.zeros(self._num_features, dtype=bool)  # 初期化を修正
    ...
    def _apply_mask(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not np.any(self._mask):  # マスクが空の場合
            return data
        mask_idx = np.where(self._mask)[0]
        return data.drop(columns=data.columns[mask_idx])
    ...
    def generate_random_mask(self) -> np.ndarray[bool, Any]:
        ...
        # マスクする特徴量のインデックスをランダムに選択
        mask_indices = self._rng.choice(
            self._num_features, 
            size=self._num_masked_features, 
            replace=False
        )
    ...
    def train_eval(self, mask: np.ndarray[bool, Any]) -> float:
        ...
        if mask.shape != (self._num_features,):
            raise ValueError(f"Mask shape must be ({self._num_features},), got {mask.shape}")
        self._mask = mask
...
# マスク率 0.5（特徴量の半分をマスク）として、TabularSvm をインスタンス化。
tabular_svm = TabularSvm(mask_ratio=0.5)

# 初期マスク（すべてFalse）で評価
initial_mask = np.zeros(tabular_svm._num_features, dtype=bool)
accuracy_score = tabular_svm.train_eval(initial_mask)
print(f"Initial accuracy score: {accuracy_score:.3f}")



# 問題サイズ（＝最適化対象のマスクの長さ）
problem_size = tabular_svm._num_features

def make_blackbox_func(tabular_svm: TabularSvm) -> Callable[[np.ndarray], float]:
    def blackbox(x: np.ndarray) -> float:
        assert x.shape == (problem_size,)
        return -tabular_svm.train_eval(x)
    return blackbox

blackbox_func = make_blackbox_func(tabular_svm=tabular_svm)

# ランダムなマスクで評価
random_mask = tabular_svm.generate_random_mask()
print(f"Random mask evaluation: {blackbox_func(random_mask):.3f}")
...
def init_training_data(
    n0: int, tabular_svm: TabularSvm, blackbox_func: Callable[[np.ndarray], float]
    ):
    """n0 組の初期教師データを作成する"""
    assert n0 < 2**problem_size

    # n0 個のマスクを乱数を用いて作成
    x = np.array([tabular_svm.generate_random_mask() for _ in range(n0)])

    # マスクの重複が発生していたらランダムに値を変更して回避する
    x = np.unique(x, axis=0)
    while x.shape[0] != n0:
        x = np.vstack((x, tabular_svm.generate_random_mask()))
        x = np.unique(x, axis=0)
...
n0 = 10  # 初期教師データの数
x, y = init_training_data(n0, tabular_svm, blackbox_func)
...
def get_constraint(q: PolyArray) -> Constraint:
    """
    マスクされる特徴量は tabular_svm.num_masked_features 個という制約条件を
    返却する関数。制約条件の重みとして 2 を選択。制約条件の重みについては、
    https://amplify.fixstars.com/ja/docs/amplify/v1/constraint.html#id8 を参照。
    """
    return 2 * equal_to(q.sum(), tabular_svm.num_masked_features)
...
def show_feature_importance(
    tabular_svm: TabularSvm,
    num_features: int = 10,
):
    """特徴量の重要度を可視化する"""
    data = tabular_svm._test_data
    labels = tabular_svm._test_labels
    assert data is not None and labels is not None
    _, test_data, _, _ = tabular_svm._fetch_masked_data()
    prediction = tabular_svm._model.predict(test_data)

    # 特徴量の重要度を計算（例：SVMの係数の絶対値）
    feature_importance = np.abs(tabular_svm._model.coef_[0])
    
    # 重要度の高い特徴量を選択
    top_features_idx = np.argsort(feature_importance)[-num_features:]
    
    plt.figure(figsize=(10, 5))
    plt.bar(range(num_features), feature_importance[top_features_idx])
    plt.xticks(range(num_features), data.columns[top_features_idx], rotation=45)
    plt.title("Feature Importance")
    plt.tight_layout()
    plt.show()

show_feature_importance(tabular_svm)
