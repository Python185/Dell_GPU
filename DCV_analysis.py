...
def dcv_analysis():
    ...
    y_file = pd.read_csv('datasets/xrd_all2.csv', encoding= 'utf-8-sig', index_col= 0)
    ...
    y_names.sort()
    #y_names = y_names[:2]
    #y_names = ['選択率NPA', '選択率エタノール']
    x_names = ['xrd_all2'] # x の設定
    ...
    #regression_model_method_normals = ['GPR_0','GPR_1']
    ...
    #各種定数、保存場所の設定
    # 保存ファイルの設定
    savedir = 'result/dcv_analysis_results/STEM_FTIR_XRD/XRD'
    ...
    for x_name in x_names:
        ...
        data = pd.read_csv(f'datasets/{x_name}.csv', index_col= 0)
